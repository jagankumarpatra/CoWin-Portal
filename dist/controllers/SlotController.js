...
exports.cancelSlot = exports.userSlotBooking = exports.BookSlot = exports.viewSlot = void 0;
...
const BookSlot = async (event) => {
    try {
        ...
        if (doseType == UserModel_1.DoseType.Two) {
            if (existingUser.vaccinationStatus !== UserModel_1.VaccinationStatus.FirstDose) {
        ...
        const existingSlotId = await slotDb?.findOne({ _id: objectId });
        console.log(existingSlotId);
        if (!existingSlotId) {
            return (0, responseFormat_1.ResponseFormat)(400, "Slot Id not found");
        }
        console.log(existingSlotId.availableCapacity);
        ...
        await slotDb?.updateOne({ _id: existingSlotId._id }, {
            $inc: { "availableCapacity": -1 }
        });
        // update booked count to 1 of that slot id
        await slotDb?.updateOne({
            _id: existingSlotId._id
        }, {
            $inc: { "bookedCount": +1 }
        });
        console.log(await slotDb?.findOne({ _id: objectId }));
        //  update slotid in bookedslot
        await db?.updateOne({ mobile: mobile }, {
            $set: {
                "bookedSlot": existingSlotId._id
            }
        });
        console.log(await db?.findOne({ mobile: mobile }));
        // update dosetype, slotid, datein doseHistory
        await db?.updateOne({ mobile: mobile }, {
            $push: {
                doseHistory: {
                    dose: doseType,
        ...
        console.log(await db?.findOne({ mobile: mobile }));
        // //1st time slot
        if (doseType === UserModel_1.DoseType.One) {
            db?.updateOne({
                mobile: mobile
            }, {
                $set: {
                    "vaccinationStatus": UserModel_1.VaccinationStatus.FirstDose
                }
            });
        }
        if (doseType === UserModel_1.DoseType.Two) {
            db?.updateOne({
                ...
                $set: {
                    "vaccinationStatus": UserModel_1.VaccinationStatus.AllDose
                }
            });
        }
        return (0, responseFormat_1.ResponseFormat)(200, `Dose ${doseType} slot booked successfully for slot ${existingSlotId}`);
    }
    catch (error) {
        console.log(error);
        return (0, responseFormat_1.ResponseFormat)(400, "Something Went Wrong", error);
...
const cancelSlot = async (event) => {
    try {
        ...
        const existingUser = await userDb?.findOne({ mobile });
        if (!existingUser) {
        ...
        console.log("$$$", slotId);
        const Info = await slotDb?.findOne({ _id: slotId });
        ...
        await slotDb?.updateOne({ _id: slotId }, {
            $inc: { "availableCapacity": -1 }
        });
        // update booked count to 1 of that slot id
        await slotDb?.updateOne({
